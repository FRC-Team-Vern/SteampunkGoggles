...
CameraServerCustom* CameraServerCustom::GetInstance() {
  static CameraServerCustom instance;
  return &instance;
}
...
std::shared_ptr<ITable> CameraServerCustom::GetSourceTable(CS_Source source) {
  std::lock_guard<std::mutex> lock(m_mutex);
  return m_tables.lookup(source);
}

std::vector<std::string> CameraServerCustom::GetSinkStreamValues(CS_Sink sink) {
  CS_Status status = 0;
...
std::vector<std::string> CameraServerCustom::GetSourceStreamValues(CS_Source source) {
  CS_Status status = 0;
  ...
  for (const auto& i : m_sinks) {
    ...
    if (source == sinkSource &&
      ...
      values.emplace_back(MakeStreamValue("10.54.61.3", port));
      break;
    }
  }
...
void CameraServerCustom::UpdateStreamValues() {
  std::lock_guard<std::mutex> lock(m_mutex);
  // Over all the sinks...
  for (const auto& i : m_sinks) {
    CS_Status status = 0;
...
CameraServerCustom::CameraServerCustom()
      ...
      [=](const cs::VideoEvent& event) {
        ...
        switch (event.kind) {
          ...
          case cs::VideoEvent::kSinkCreated:
			std::cout << "kSinkCreated:" << std::endl;
...
cs::UsbCamera CameraServerCustom::StartAutomaticCapture() {
  return StartAutomaticCapture(m_defaultUsbDevice++);
}

cs::UsbCamera CameraServerCustom::StartAutomaticCapture(int dev) {
  llvm::SmallString<64> buf;
...
cs::UsbCamera CameraServerCustom::StartAutomaticCapture(llvm::StringRef name,
                                                  int dev) {
...
cs::AxisCamera CameraServerCustom::AddAxisCamera(llvm::StringRef host) {
  return AddAxisCamera("Axis Camera", host);
}

cs::AxisCamera CameraServerCustom::AddAxisCamera(const char* host) {
  return AddAxisCamera("Axis Camera", host);
}

cs::AxisCamera CameraServerCustom::AddAxisCamera(const std::string& host) {
  return AddAxisCamera("Axis Camera", host);
}

cs::AxisCamera CameraServerCustom::AddAxisCamera(llvm::ArrayRef<std::string> hosts) {
  return AddAxisCamera("Axis Camera", hosts);
}

cs::AxisCamera CameraServerCustom::AddAxisCamera(llvm::StringRef name,
                                           llvm::StringRef host) {
...
void CameraServerCustom::StartAutomaticCapture(const cs::VideoSource& camera) {
...
cs::CvSink CameraServerCustom::GetVideo() {
  cs::VideoSource source;
  {
    std::lock_guard<std::mutex> lock(m_mutex);
    if (m_primarySourceName.empty()) {
      std::cerr << "no camera available" << std::endl;
      // wpi_setWPIErrorWithContext(CameraServerCustomError, "no camera available");
      return cs::CvSink{};
    }
...
cs::CvSink CameraServerCustom::GetVideo(const cs::VideoSource& camera) {
...

  {
    ...
    if (it != m_sinks.end()) {
      ...
      if (kind != cs::VideoSink::kCv) {
        ...
        // wpi_setWPIErrorWithContext(CameraServerCustomError, err.str());
        return cs::CvSink{};
      }
...
#if 0
cs::CvSink CameraServerCustom::GetVideo(llvm::StringRef name) {
  cs::VideoSource source;
  {
    std::lock_guard<std::mutex> lock(m_mutex);
...
cs::CvSource CameraServerCustom::PutVideo(llvm::StringRef name, int width,
                                    int height) {
...
cs::MjpegServer CameraServerCustom::AddServer(llvm::StringRef name) {
  int port;
  {
    std::lock_guard<std::mutex> lock(m_mutex);
    port = m_nextPort++;
...
cs::MjpegServer CameraServerCustom::AddServer(llvm::StringRef name, int port) {
  cs::MjpegServer server{name, port};
  AddServer(server);
  return server;
}

void CameraServerCustom::AddServer(const cs::VideoSink& server) {
  std::lock_guard<std::mutex> lock(m_mutex);
  m_sinks.emplace_second(server.GetName(), server);
}

void CameraServerCustom::RemoveServer(llvm::StringRef name) {
  std::lock_guard<std::mutex> lock(m_mutex);
  m_sinks.erase(name);
}

#if 0
cs::VideoSink CameraServerCustom::GetServer() {
  llvm::SmallString<64> name;
  {
    std::lock_guard<std::mutex> lock(m_mutex);
    if (m_primarySourceName.empty()) {
...
cs::VideoSink CameraServerCustom::GetServer(llvm::StringRef name) {
  std::lock_guard<std::mutex> lock(m_mutex);
  auto it = m_sinks.find(name);
  if (it == m_sinks.end()) {
    llvm::SmallString<64> buf;
...
void CameraServerCustom::AddCamera(const cs::VideoSource& camera) {
...
void CameraServerCustom::RemoveCamera(llvm::StringRef name) {
  std::lock_guard<std::mutex> lock(m_mutex);
  m_sources.erase(name);
}

void CameraServerCustom::SetSize(int size) {
  std::lock_guard<std::mutex> lock(m_mutex);
